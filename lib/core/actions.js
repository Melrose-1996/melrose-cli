...
const createProject = async (project, otherArg) => {
  // 1.提示信息
  log.hint(
    'melrose helps you create your project, please wait a moment~'
  );
...
const addVue3TSComponent = async (name, dest) => {
  handleEjsToFile(
    name,
    dest,
    '../template/component3_ts.vue.ejs',
    `${name}.vue`
  );
};
...
const addVue3PageSetup = async (name, dest) => {
  handleEjsToFile(
    name,
    dest,
    '../template/component3_ts_su.vue.ejs',
    `${name}.vue`
  );
  let routerDest = dest.replace('views', 'router');
  handleEjsToFile(
...
const addStore = async (name, dest) => {
  handleEjsToFile(
    name,
    dest,
    '../template/vue-store.js.ejs',
    'index.js'
  );
  handleEjsToFile(
    name,
    dest,
    '../template/vue-types.js.ejs',
    'types.js'
  );
};
